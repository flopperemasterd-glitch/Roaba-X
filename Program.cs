...
class RoabaPipe
{
    ...
    static void PipeScanValue(string data)
    {
        ...
        lock (RoabaX.ConsoleLock) Console.WriteLine($"[PIPE] Scanning for value: {valueToFind}");
...
namespace Roaba_Matii
{
    ...
    class RoabaX
    {
        ...
        [DllImport("kernel32.dll", SetLastError = true)] public static extern bool VirtualProtectEx(IntPtr hProcess, IntPtr lpAddress, UIntPtr dwSize, uint flNewProtect, out uint lpflOldProtect);
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                Thread.Sleep(500);
                processHandle = OpenProcess(PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION, false, targetProcess.Id);
                ...
                while (running)
                {
                    ...
                    Console.WriteLine("[6] ?? Țevi server (Executie cu telecomanda)");
            ...
            catch (IndexOutOfRangeException) { TippedOver("Ia zi ceai scris prost"); }
            catch (OutOfMemoryException) { LoadTooHeavy("Nu mai avem spatiu in roaba"); }
            catch (Exception ex) { TippedOver($"'nu stim' de ce tia cazut roaba: {ex.Message}"); }
        ...
        static void UnprotectFunctionMenu()
        {
            ...
            catch (Exception ex) { AxleSqueaking($"Unprotect failed: {ex.Message}"); }
        }
        ...
        static void AdvancedPatternMenu()
        {
            try
            {
                ...
                else Console.WriteLine("[-] Pattern nu a fost gasit");
            }
            catch (Exception ex) { AxleSqueaking($"Scan failed: {ex.Message}"); }
        }
        ...
        static string GetProtectionString(uint protect)
        {
            ...
            if ((protect & 0x01) != 0) result += "N/A";      // PAGE_NOACCESS
            ...
            if ((protect & 0x80) != 0) result += "WXC";      // PAGE_EXECUTE_WRITECOPY

            if ((protect & 0x100) != 0) result += " (Guard)";   // PAGE_GUARD
        ...
        static void MultiScanPattern()
        {
            ...
            catch (Exception ex) { AxleSqueaking($"Multi-scan failed: {ex.Message}"); }
        }
        ...
        static void ReadMemory()
        {
            ...
            catch (Exception ex) { AxleSqueaking($"Ce ai mai facut prost: {ex.Message}"); }
        }
        ...
        static void WriteMemory()
        {
            try
            {
                ...
                if (!protectChanged) AxleSqueaking("Nu pot sa schimb protectia memoriei");
            ...
            catch (Exception ex) { TippedOver($"Sa taco bell roaba: {ex.Message}"); }
        }
        ...
        static void ScanForValue()
        {
            try
            {
                ...
                for (long offset = 0; offset < moduleSize; offset += chunkSize - 3)
                {
                    ...
                    if (offset % (moduleSize / 20) == 0) Console.Write("█");
                }
                ...
                if (results.Count > 0)
                {
                    ...
                    if (results.Count > 20) Console.WriteLine($" ... si {results.Count - 20} mai multe");
                }
            }
            catch (Exception ex) { AxleSqueaking($"Ti se farama roaba ai grija: {ex.Message}"); }
        }
        ...
        static void PatternScan()
        {
            try
            {
                ...
                if (results.Count > 0)
                {
                    ...
                    for (int i = 0; i < Math.Min(results.Count, 10); i++)
                    {
                        ...
                        {
                            if (pattern[j].HasValue) { Console.ForegroundColor = ConsoleColor.Cyan; Console.Write($"{foundBytes[j]:X2} "); Console.ResetColor(); }
                            else { Console.ForegroundColor = ConsoleColor.Yellow; Console.Write($"{foundBytes[j]:X2} "); Console.ResetColor(); }
                        }
                        Console.WriteLine();
                    }
                    if (results.Count > 10) Console.WriteLine($"\n ... si inca {results.Count - 10} rezultate");
                    ...
                    if (!string.IsNullOrEmpty(choice) && int.TryParse(choice, out int index) && index >= 0 && index < results.Count)
                    {
                        ...
                        else if (action == "2")
                        {
                            ...
                            else WheelFellOff("Nu a mers scrierea");
                        }
                    }
                }
                else
            ...
            catch (Exception ex) { TippedOver($"Pattern scan a dat cu roaba peste cap: {ex.Message}"); }
        }
        ...
        static void DisplayHexDump(IntPtr startAddress, byte[] buffer, int bytesRead)
        {
            ...
            for (int i = 0; i < bytesRead; i += 16)
            {
                ...
                for (int j = bytesRead - i; j < 16; j++) Console.Write(" ");
                Console.Write(" ");
                for (int j = 0; j < 16 && i + j < bytesRead; j++)
                {
        ...
        static void ShowSplashScreen()
        {
            ...
            Console.WriteLine("║    versuiunea: 2.Roaba Cu Motor Disel   ║");
            Console.WriteLine("║         florin salam on top             ║");
            Console.WriteLine("╚═════════════════════════════════════════╝");
            Console.ResetColor();
            Console.WriteLine();
            Console.Write("[+] Loading wheel... ");
            for (int i = 0; i <= 100; i += 10) { Console.Write("█"); Thread.Sleep(50); }
